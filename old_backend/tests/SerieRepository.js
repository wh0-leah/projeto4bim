/*
  MODELO IMPORTADO:
  Pega o modelo 'Serie' que possui informações específicas como criador, temporadas,
...
export class SerieRepository {
  ...
  /*
    ...
    Filtra as séries na nossa lista por gênero, criador, elenco, título e status.
    Devolve em ordem decrescente de nota média de avaliação.
  */
  static listar({ genero, criador, ator, titulo, status } = {}) {
    return this.collection
      .filter(s => !titulo || s.titulo.toLowerCase().includes(titulo.toLowerCase()))
      .filter(s => !criador || s.criador.toLowerCase().includes(criador.toLowerCase()))
