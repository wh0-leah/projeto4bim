...
export async function excluirNomeCadastro(req, res, next) {
  try {
    const id = req.body.id || req.query.id || req.params.id;
    if (!id) throw new Error('Chave primária não informada.');

    const resultado = await filmeService.removerFilme(id, req.usuario.id, req.usuario.role);
    res.json(resultado);
  } catch (err) { next(err); }
}
